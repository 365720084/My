    ...
    public static void check(Context context) {
        ...
        if (var0 != null) {
            if (Logger.checkDebugOpen()) {
                ...
                Logger.e("PtgAdSdk-check", "==当前进程名：" + var1);
                Logger.e("PtgAdSdk-check", "==PtgAdsdk接入，环境为debug，初始化配置检测开始==");
                    ...
                    while(true) {
                        ...
                        while(var8.hasNext()) {
                            ...
                            if ("com.bytedance.sdk.openadsdk.multipro.TTMultiProvider".equals(var9.name)) {
                                ...
                                if (!TextUtils.isEmpty(var25) && var25.equals(var9.authority)) {
                                    Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTMultiProvider配置正常");
                                } else {
                                    Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTMultiProvider配置异常：android:authorities，请参考接入文档");
                                }
                            } else if (var9.authority.equals(var2 + ".TTFileProvider")) {
                                var7 = true;
                                if (VERSION.SDK_INT >= 24 && var3 >= 24) {
                                    if (var9.exported) {
                                        Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTFileProvider配置异常：android:exported，请参考接入文档");
                                    }

                                    if (!var9.grantUriPermissions) {
                                        Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTFileProvider配置异常：android:grantUriPermissions，请参考接入文档");
                                    }
                                    ...
                                    try {
                                        ...
                                        if (var14 != null && !var14.isEmpty()) {
                                            ...
                                            if (var15.isEmpty() && var16.isEmpty()) {
                                                Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTFileProvider配置正常");
                                            } else {
                                                var17 = var15.iterator();

                                                while(var17.hasNext()) {
                                                    var18 = (EnviromentHelper.a)var17.next();
                                                    Logger.e("PtgAdSdk-check", "    TTFileProvider缺少必要路径：" + var18.toString());
                                                }
                                                ...
                                                while(var17.hasNext()) {
                                                    var18 = (EnviromentHelper.a)var17.next();
                                                    Logger.e("PtgAdSdk-check", "    TTFileProvider缺少可选路径：" + var18.toString());
                                                }
                                            }
                                        } else {
                                            Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTFileProvider中路径配置异常，请参考接入文档");
                                        }
                                    } catch (Throwable var21) {
                                        Logger.e("PtgAdSdk-check", "AndroidManifest.xml中TTFileProvider配置错误，请参考接入文档", var21);
                                    }
                                } else {
                                    Logger.e("PtgAdSdk-check", "TTFileProvider不需要适配：target=" + var3 + "&phone=" + VERSION.SDK_INT + ", require=" + 24);
                                }
                            }
                        }
                        ...
                        try {
                            ...
                            if (var26 != null && var26.length > 0) {
                                ...
                                if (var28.isEmpty()) {
                                    Logger.e("PtgAdSdk-check", "AndroidManifest.xml中权限配置正常");
                                } else {
                                    Iterator var31 = var28.iterator();

                                    while(var31.hasNext()) {
                                        String var33 = (String)var31.next();
                                        Logger.e("PtgAdSdk-check", "    可能缺少权限：" + var33 + "，请参考接入文档");
                                    }
                                }
                            } else {
                                Logger.e("PtgAdSdk-check", "AndroidManifest.xml中uses-permission配置丢失，请参考接入文档");
                            }
                        } catch (Throwable var20) {
                            Logger.e("PtgAdSdk-check", "AndroidManifest.xml中uses-permission配置错误，请参考接入文档", var20);
                        }
...
//                                if (!var24) {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限没有获取，可能影响转化：android.permission.READ_PHONE_STATE");
//                                } else {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限正常：android.permission.READ_PHONE_STATE");
//                                }
//
//                                if (!var27) {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限没有获取，可能影响转化：android.permission.ACCESS_COARSE_LOCATION");
//                                } else {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限正常：android.permission.ACCESS_COARSE_LOCATION");
//                                }
//
//                                if (!var29) {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限没有获取，可能影响转化：android.permission.ACCESS_FINE_LOCATION");
//                                } else {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限正常：android.permission.ACCESS_FINE_LOCATION");
//                                }
//
//                                if (!var32) {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限没有获取，可能影响转化：android.permission.WRITE_EXTERNAL_STORAGE");
//                                } else {
//                                    Logger.oepnDebug("PtgAdSdk-check", "动态权限正常：android.permission.WRITE_EXTERNAL_STORAGE");
//                                }
//                            } else {
//                                Logger.oepnDebug("PtgAdSdk-check", "动态权限不需要适配：target=" + var3 + "&phone=" + VERSION.SDK_INT + ", require=" + 23);
//                            }
                        } catch (Throwable var19) {
                            Logger.e("PtgAdSdk-check", "动态权限获取异常，请检查并详细阅读接入文档", var19);
                        }

                        if (!var6) {
                            Logger.e("PtgAdSdk-check", "××您没有配置TTMultiProvider，请参考接入文档，否则影响转化××");
                        }

                        if (!var7) {
                            Logger.e("PtgAdSdk-check", "××您没有配置TTFileProvider，请参考接入文档，否则影响转化××");
                        }

                        if (!var22) {
                            Logger.e("PtgAdSdk-check", "××您没有配置permission，请参考接入文档，否则影响转化××");
                        }

                        Logger.e("PtgAdSdk-check", "==PtgAdsdk初始化配置检测结束==");
                        return;
                    }
                }
            }
